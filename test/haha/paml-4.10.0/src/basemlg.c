...
int Forestry(FILE* fout, double space[])
{
   ...
   for (itree = 0; itree < ntree; itree++) {
      ...
      */
      ...
      fprintf(fout, "\nlnL(np:%3d):%14.6f%+12.6f\n", com.np, -lnL[itree], -lnL[itree] + lnL[0]);
      OutTreeB(fout);
      fprintf(fout, "\n");
...
int testx(double x[], int np)
{
   ...
   for (i = 0; i < com.ntime; i++)  if (x[i]<tb[0] || x[i]>tb[1])   return (-1);
   ...
   */
   ...
   if (com.clock) {
      ...
      for (i = 0; i < com.ntime; i++)
         for (j = 0; j < 2; j++)
            xb[i][j] = tb[j];
   for (i = 0; i < com.nrgene; i++) for (j = 0; j < 2; j++)
      xb[com.ntime + i][j] = rgeneb[j];
   for (i = 0; i < com.nrate; i++)  for (j = 0; j < 2; j++)
...
int GetInitials(double x[], int* fromfile)
{
   ...
   if (com.model == REV) {
      ...
      for (i = 0; i < com.nrate - 1; i++)
         x[com.ntime + com.nrgene + i + 1] = 1 / com.kappa;
   }
...
int lfunG_print(double x[], int np)
{
   ...
   */
   ...
   for (h = 0, lnL = 0, igene = -1, lt = 0; h < com.npatt; lt += (int)com.fpatt[h++]) {
      ...
      for (i = 0; i < com.ns; i++)
         fprintf(flfh, "%c", BASEs[(int)com.z[i][h]]);
   ...
   for (h = 0; h < com.ls; h++) {
      ...
      for (j = 0; j < com.ns; j++)
         fprintf(frate, "%c", BASEs[(int)com.z[j][hp]]);
...
double lfunG(double x[], int np)
{
   ...
   */
   ...
   int  im, nm, nsum = 1 << (2 * (tree.nnode - com.ns));
   int  ninter = tree.nbranch - com.ns + 1;
   double lnL, fh, Bmx, y, alpha, kappa;
   ...
   for (h = 0, igene = -1, lt = 0; h < com.npatt; lt += (int)com.fpatt[h++]) {
      ...
      for (im = 0, fh = 0, zero(dfh, np), zero(ddfh, np* np); im < nm; im++) {
         ...
         if (!com.fix_alpha) {
            ...
            for (j = 0; j < nbranch; j++)
               ddfh[(np - 1) * np + j] += c * Root[M[j]] * y2;  /* at */
            if (com.ngene > 0 && igene > 0)
