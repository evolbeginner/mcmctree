   ...
   double U[], double V[], double space[])
{
   ...
   if (n <= 4) {
      ...
      for (i = 0; i < n; i++) {
         for (j = 0; j < n; j++) fprintf(fout, "%6.0f", Q[i * n + j] * 100);
         fprintf(fout, "\n");
      }
      /*
      for (i = 0; i < n; i++) {
         fprintf (fout,"\n%-4s", getAAstr(aa3,i));
         for(j=0; j<i; j++) fprintf (fout, "%4.0f", Q[i*n+j]/pi[j]*100);
         fprintf (fout, "%4.0f", -Q[i*n+i]*100);
      }
      fputs("\n     ",fout);
      for(i=0; i<naa; i++) fprintf(fout,"%5s",getAAstr(aa3,i));
      */
      fprintf(fout, "\n\nPAM matrix, P(0.01)\n");
      for (i = 0; i < n; i++) 
         for (j = 0; j < n; j++)
            T1[i * n + j] = U[i * n + j] * exp(0.01 * Root[j]);
      matby(T1, V, Q, n, n, n);
      for (i = 0; i < n*n; i++)
         if (Q[i] < 0) Q[i] = 0;
      for (i = 0; i < n; i++) {
         fprintf(fout, "\n%-4s", getAAstr(aa3, i));
         for (j = 0; j < n; j++)
            fprintf(fout, "%6.0f", Q[i*n + j] * 10000);
      }
      fputs("\n     ", fout);  
      for (i = 0; i < n; i++) 
   ...
   double Root[], double U[], double V[], double space[])
{
   ...
   */
   ...
   for (k = 0; k < tree.nbranch; k++) {
      ...
      for (i = 0; i < n; i++) 
         for (j = 0; j < n; j++)
            T1[i*n + j] = U[i*n + j] * exp(Root[j] * branch[k]);
      matby(T1, V, P, n, n, n);
      for (i = 0; i < n*n; i++)   
         if (P[i] < 0) P[i] = 0;
      /*
      ...
      for (npath = 0, sumpr = bestpr = 0; ; ) {
         for (j = 0, k = visit[nid - 1]; j < NCharaCur[k]; j++) {
            ...
            if (job == 1) {
               ...
               sumpr += pr;
               for (i = 0; i < nid; i++)
                  pnsite[i * n + nodeb[i + com.ns]] += pr;
               if (pr > bestpr) {
                  bestpr = pr;
                  for (i = 0; i < nid; i++)
                     bestPath[i] = PATHWay[i];
               }
            }
            else {
...
*/
      if (job == 0)
         ...
         if (npath > 1) {
            for (i = 0; i < nid; i++)
               fprintf(fout, "%c", pch[(int)bestPath[i]]);
   ...
   if (job == 0) {
      ...
      for (i = 0; i < tree.nbranch; i++)
         for (j = 0; j < n * n; j++)
            Ft[tree.nbranch * n * n + j] += Ft[i * n * n + j];
   }
   else {
   ...
   double V[], double pi[], double space[], int *cond)
{
   ...
   */
   ...
   double *Q = Ft, *T1 = space, t, pi_sqrt[20], small = 0.1 / com.ls;
