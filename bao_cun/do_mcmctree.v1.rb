...
PAML_DIR = File.expand_path("~/software/phylo/paml4.9i/")
...
`#{MCMCTREE} mcmctree.ctl >> #{outdir}/mcmctree.first`
...
if seqtype == 2
  ...
  1.upto(ndata).each do |i|
    ...
    prepare_paml_ctl(File.join(outdir, b), \
      outdir, {'model'=>2,'aaRatefile'=>AA_RATE_FILE,'fix_alpha'=>0,'alpha'=>alpha, 'ncatG'=>ncatG}, {})
    `codeml #{b}`
    `cat rst2 >> in.BV`
  end
  Dir.chdir($PWD)
